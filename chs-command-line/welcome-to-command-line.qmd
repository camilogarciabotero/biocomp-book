...
There are basically **two** ways to explore or navigate your file system. If you always reprent it from the root, then you are presenting an **absolute path**. For instance the absoulte path to my desktop is (`/Users/camilogarcia/Desktop`). 
...
::: {.callout-tip}
When having a long command, it becomes practically to go to the begining or to the end of it. To do so you can use the key combination `Ctrl + A` and `Ctrl + E` respectively.
...
There is still many conventions by wich the parts of a command line might be called, yet a very standard convention is presented in [Fig. @fig-command]
