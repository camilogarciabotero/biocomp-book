...
To enable the communication with a remote repo, git has encoded many specific commands, once the repos are **cloned** a simple workflow from the own local and remote repos is made possible thanks to three simple commands `git push` and `git pull`. 

::: {.column-margin}
![The octocat from GitHub](imgs-cli/octocat.jpeg)
:::
